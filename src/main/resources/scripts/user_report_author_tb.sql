...
BEGIN
    ...
    if (select exists(select 1 from user_tb ub where ub.user_id = author_id_param and is_author = true)) then
        if (select exists(select 1
                          from user_tb ub
                                   inner join user_role_tb urb on ub.user_id = urb.user_id
                                   inner join role_tb rb on rb.role_id = urb.role_id
                          where ub.user_id = author_id_param
                            and rb.name = 'ROLE_AUTHOR')) then
        ...
        elseif (select exists(select 1
                              from user_tb ub
                                       ...
                                       inner join role_tb rb on rb.role_id = urb.role_id
                              where ub.user_id = author_id_param
                                and rb.name = 'ROLE_ADMIN')) then
            raise exception 'This user is not author.';
        end if;
    else
        raise exception 'No author_id had been found.';
    end if;
end;
$$ language plpgsql;
...
select exists(select 1
              from user_tb ub
                       inner join user_role_tb urb on ub.user_id = urb.user_id
                       inner join role_tb rb on rb.role_id = urb.role_id
              where ub.user_id = '1a816fc1-90cb-480a-93ed-6b5e21322bd0'
                and rb.name = 'ROLE_AUTHOR');
select exists(select 1
              from user_tb
                       inner join user_role_tb urb on user_tb.user_id = urb.user_id
                       inner join role_tb rb on urb.user_id = user_tb.user_id
              where is_author = true
                and user_tb.user_id = '1a816fc1-90cb-480a-93ed-6b5e21322bd0');
...
set status = 'APPROVED'
where urab.author_id = '3ec5231c-f6e6-4a03-aa8f-9b99b1c5a6b9'
returning urab.author_id;
